...
class ResourceGraph extends Component {
  ...
  openInEditor(event) {
  	const resource = event.target.scratch()._resource;
  ...
  cyRef(cy) {
    ...
    cy.on('taphold', 'node', (evt) => {
      ...
      const elements = []; // the nodes (artifacts) and edges (links) we want to add to the graph
      // GET the resource, we currently only have its Compact representation
      server.read(node.id(), (err, resource) => {
        if (err) {
        ...
        linkTypes.forEach((link) => {
          ...
          async.forEach(targets, (targetURI, callback) => {
            ...
            // add the new elements (nodes and edges) to the graph
			      cy.add(elements);
			      const layout = cy.layout({ name: 'dagre', fit: false });
			      layout.run();
			    });
      	}); // forEach link
    	}); // read
